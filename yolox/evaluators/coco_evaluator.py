...
class COCOEvaluator:
    ...
    def evaluate(
        ...
        if trt_file is not None:
            ...
            x = torch.ones(1, 3, test_size[0], test_size[1]).to(xm.xla_device())
            model(x)
            model = model_trt
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                #imgs = imgs.to(xm.xla_device()).type(tensor_type)
                imgs = imgs.to(xm.xla_device(), dtype=tensor_type)
        ...
        statistics = torch.FloatTensor([inference_time, nms_time, n_samples]).to(xm.xla_device())
